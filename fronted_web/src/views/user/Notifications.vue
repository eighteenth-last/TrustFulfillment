<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-800">通知消息</h1>
    ...
    <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
      <n-spin :show="loading">
        <div v-if="filteredNotifications.length === 0" class="py-20 text-center">
          <i class="far fa-bell-slash text-5xl text-gray-300 mb-4"></i>
          <p class="text-gray-400">暂无通知消息</p>
        </div>
        <div v-else class="divide-y divide-gray-100">
          <div 
            ...
            class="p-5 hover:bg-gray-50 cursor-pointer transition-all"
            :class="{ 'bg-blue-50/30': !noti.isRead }"
          >
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-3">
                  <h3 class="font-bold text-gray-800">{{ noti.title }}</h3>
                  <n-tag v-if="!noti.isRead" size="tiny" type="info">未读</n-tag>
                </div>
                <p class="text-gray-500 mt-1">{{ noti.content }}</p>
